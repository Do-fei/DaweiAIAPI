...
import { InsertUser, users, apiKeys, transactions, conversations, messages, models } from "../drizzle/schema";
...
// ============ 交易查询 ============
export async function getTransactionsByUserId(userId: number, limit: number = 50, offset: number = 0) {
  const db = await getDb();
  if (!db) return [];
...
export async function getActiveModels() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(models).where(eq(models.status, "active"));
}

export async function getModelByName(modelName: string) {
  const db = await getDb();
  if (!db) return undefined;
  const result = await db.select().from(models).where(eq(models.modelName, modelName)).limit(1);
  return result[0];
}
